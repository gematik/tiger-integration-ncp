...
Funktionalität: NCPeH PSA Use Case 2 - Verfügbare Versichertendatensätze des ePKA MIO auflisten
  AF_10121 - Verfügbare Versichertendatensätze des ePKA MIO auflisten
    ...
    In diesem zweiten Use Case wird der LE-EU das/die Document Identifier des Patient Summaries ("PS")
    der versicherten Person ermitteln. Der NCPeH-FD wird Document Identifier des PS für zwei
    Ausprägungsformen ausliefern - PS in Form eines strukturierten und transcodierten Dokumentes
    (CDA Level 3) und in Form eines PDF-Dokumentes nur mit originalen Daten (CDA Level 1).
    Aktivität des NCPeH-FD:
    Anhand der im Request des NCPeH Land B übergebenen Daten sucht der NCPeH-FD im ePA Aktenkonto
    das ePKA-Dokument der versicherten Person und erweitert die gefundene DocumentId um die
    Informationen für CDA L3 und CDA L1 und gibt beide Identifier in der Antwort zurück.
  ...
  Szenario: NCPeH - PSA UC2 Suche nach den verfügbaren Patient Summary Dokumenten - es wird kein ePKA Dokument gefunden
    Die Identifikation des Patienten wurde vom LE-EU erfolgreich durchgeführt und die Durchführung
    einer Behandlung akzeptiert und der NCPeH Land B hat intern eine Treatment Relationship Confirmation (TRC) erstellt.
    Im Aktenkonto des Versicherten wurde im Zeitraum zwischen der Identifikation des Patienten und dem Start der Suche
    ...
    Der NCPeH gibt eine Antwort gemäß TUC_NCPeH_013 für XCA-Operationen mit Fehlercode ERROR_GENERIC_DOCUMENT_MISSING
    zurück. (siehe TAB_NCPeH_Abruf_ePKA_Metadaten_Fehlerbehandlung_Zusammenhang_XCA entsprechend Fehlerkorrektur im
    Change C_11967 zum NCPeH-FD v2.0)
