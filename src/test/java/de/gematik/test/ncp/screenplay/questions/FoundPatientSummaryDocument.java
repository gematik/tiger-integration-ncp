...
public class FoundPatientSummaryDocument implements Question<Boolean> {

  private final Integer level;

  public FoundPatientSummaryDocument(final Integer level) {
    this.level = level;
  }
  ...
  public Boolean answeredBy(final Actor actor) {
    ...
    final var documentMetadata =
        IheUtils.readDocumentMetadataFromAdhocQueryResponse(patientData.getPsaMetadata(), level);
    return Objects.nonNull(documentMetadata);
  }

  public static FoundPatientSummaryDocument ofCdaLevel(final Integer level) {
    return new FoundPatientSummaryDocument(level);
  }
}
