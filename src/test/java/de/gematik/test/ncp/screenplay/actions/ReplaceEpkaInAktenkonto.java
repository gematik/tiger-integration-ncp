...
public class ReplaceEpkaInAktenkonto implements Performable {
  @Override
  public <T extends Actor> void performAs(final T actor) {
    ...
    final var practice = actor.usingAbilityTo(ProvidePracticeData.class);
    final var documentId = ps.replaceDocument(patientData.kvnr(), practice, epka);
    patientData.setEpkaAktenkontoId(documentId);
    ps.addKvnrToCleanEpka(patientData.kvnr());
  }

  public static ReplaceEpkaInAktenkonto instance() {
    return Instrumented.instanceOf(ReplaceEpkaInAktenkonto.class).newInstance();
  }
}
